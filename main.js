let App_ID = "2abd61194f4a40d592a7bd0ef6e1e553"
...
let handleUserLeft = (MemberId) => {
    ...
    document.getElementById("user1").classList.remove("frame");
}
...
let createPeerConnection = async (MemberId) => {
    ...
    document.getElementById("user2").srcObject = remote;
...
let toggleCamera = async () => {
    let videoTrack = local.getTracks().find(track => track.kind === 'video');
...
let toggleMic = async () => {
    let audioTrack = local.getTracks().find(track => track.kind === 'audio');
